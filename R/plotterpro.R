...
plotterpro <- function() {
  ...
  printSuccess <- function() {
    ...
    if (sum(mydata$sex == "M") > 0) {
      mydata$SDS_gen <- round(
        calculateSDS(pub.data$age, pub.data$gen, "gen"),
        2
      )
      mydata$SDS_phb <- round(
        calculateSDS(pub.data$age, pub.data$phb, "phb"),
        2
      )
    ...
    if (sum(mydata$sex == "F") > 0) {
      mydata$SDS_bre <- round(
        calculateSDS(pub.data$age, pub.data$bre, "bre"),
        2
      )
      mydata$SDS_phg <- round(
        calculateSDS(pub.data$age, pub.data$phg, "phg"),
        2
      )
      mydata$SDS_men <- round(
        calculateSDS(pub.data$age, pub.data$men, "men"),
        2
      )
    }
