...
upload_tryCatch_pro <- function() {
  ...
  printSuccess <- function() {
    ...
    if (is.null(mydata$id)) {
      stop(
        "Variabable header 'id' not found in the data! This variable is required!"
      )
    }
    if (is.null(mydata$age)) {
      stop(
        "Variabable header 'age' not found in the data! This variable is required!"
      )
    }
    if (is.null(mydata$sex)) {
      stop(
        "Variabable header 'sex' not found in the data! This variable is required!"
      )
    }
    ...
    if (sum(is.na(mydata$id)) > 0) {
      ...
      stop(paste(
        "Variable 'id' contains missing values! This is not allowed. Rownumbers:",
        toString(missings)
      ))
    }
    ...
    if (sum(is.na(mydata$age)) > 0) {
      ...
      stop(paste(
        "Variable 'age' contains missing values! This is not allowed. Rownumbers:",
        toString(missings)
      ))
    }
    ...
    if (sum(is.na(mydata$sex)) > 0) {
      ...
      stop(paste(
        "Variable 'sex' contains missing values! This is not allowed. Rownumbers:",
        toString(missings)
      ))
    }
    ...
    if (sum(!(mydata$sex == "M" | mydata$sex == "F")) > 0) {
      ...
      stop(paste(
        "Variable 'sex' contains invalid values! It should only conain F or M. Rownumbers:",
        toString(wrongvalues)
      ))
    }

    if (sum(mydata$sex == "M") > 0) {
      #there are boys in the datafile

      if (is.null(mydata$tv)) {
        stop(
          "Variabable header 'tv' not found in the data! This variable is required for boys!"
        )
      }
      if (is.null(mydata$gen)) {
        stop(
          "Variabable header 'gen' not found in the data! This variable is required for boys!"
        )
      }
      if (is.null(mydata$phb)) {
        stop(
          "Variabable header 'phb' not found in the data! This variable is required for boys!"
        )
      }

      invalidtv <- !(mydata$tv == 2 |
        mydata$tv == 3 |
        mydata$tv == 4 |
        mydata$tv == 8 |
        mydata$tv == 12 |
      ...
      if (sum(invalidtv) > 0) {
        stop(paste(
          "Variable 'tv' contains invalid values. It should have only values 2,3,4,8,12,16,20,25 or NA. Row numbers:",
          toString(rownames(mydata)[invalidtv])
        ))
      }

      invalidgen <- !(mydata$gen == 1 |
        mydata$gen == 2 |
        mydata$gen == 3 |
        mydata$gen == 4 |
        mydata$gen == 5 |
      ...
      if (sum(invalidgen) > 0) {
        stop(paste(
          "Variable 'gen' contains invalid values. It should have only values 1,2,3,4,5 or NA. Row numbers:",
          toString(rownames(mydata)[invalidgen])
        ))
      }

      invalidphb <- !(mydata$phb == 1 |
        mydata$phb == 2 |
        mydata$phb == 3 |
        mydata$phb == 4 |
        mydata$phb == 5 |
      ...
      if (sum(invalidphb) > 0) {
        stop(paste(
          "Variable 'phb' contains invalid values. It should have only values 1,2,3,4,5 or NA. Row numbers:",
          toString(rownames(mydata)[invalidphb])
        ))
      }
    }

    if (sum(mydata$sex == "F") > 0) {
      #there are girls in the datafile

      if (is.null(mydata$bre)) {
        stop(
          "Variabable header 'bre' not found in the data! This variable is required for girls!"
        )
      }
      if (is.null(mydata$phg)) {
        stop(
          "Variabable header 'phg' not found in the data! This variable is required for girls!"
        )
      }
      if (is.null(mydata$men)) {
        stop(
          "Variabable header 'men' not found in the data! This variable is required for girls!"
        )
      }
      ...
      if (sum(invalidmen) > 0) {
        stop(paste(
          "Variable 'men' contains invalid values. It should have only values 1,2 or NA. Row numbers:",
          toString(rownames(mydata)[invalidmen])
        ))
      }

      invalidbre <- !(mydata$bre == 1 |
        mydata$bre == 2 |
        mydata$bre == 3 |
        mydata$bre == 4 |
        mydata$bre == 5 |
      ...
      if (sum(invalidbre) > 0) {
        stop(paste(
          "Variable 'bre' contains invalid values. It should have only values 1,2,3,4,5 or NA. Row numbers:",
          toString(rownames(mydata)[invalidbre])
        ))
      }

      invalidphg <- !(mydata$phg == 1 |
        mydata$phg == 2 |
        mydata$phg == 3 |
        mydata$phg == 4 |
        mydata$phg == 5 |
      ...
      if (sum(invalidphg) > 0) {
        stop(paste(
          "Variable 'phg' contains invalid values. It should have only values 1,2,3,4,5 or NA. Row numbers:",
          toString(rownames(mydata)[invalidphg])
        ))
      }
    }
    ...
    for (i in 1:length(persons)) {
      ...
      if (length(unique(allsex)) > 1) {
        stop(paste(
          "Person with id",
          pnr,
          "has a different sex on varying records. Please fix this."
        ))
      }
    }
  ...
  tryCatch(printSuccess(), error = function(e) {
    printFailure(e)
  })
}
