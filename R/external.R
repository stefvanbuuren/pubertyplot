...
plot_stadia <- function(
  ...
  for (i in 1:length(persons)) {
    ...
    if (any(select, na.rm = TRUE)) {
      ...
      if (sex == "M") {
        pubs <- data[select, c("gen", "phb", "tv")]
      } else {
        pubs <- data[select, c("bre", "phg", "men")]
      }
...
plot_stadia_lines <- function(plotlines, colors, sex, ref) {
  ...
  if (sex == "M") {
    if (plotlines[3]) {
      ...
      text(
        x = c(9, 9, 9, 9, 9),
        y = c(-0.65, 0.75, 1.5, 2.2, 2.6),
        labels = c("2ml", "3ml", "4ml", "8ml", "12ml"),
        cex = 0.8,
        adj = 0,
        col = col
      )
...
plot_stadia_data <- function(ages, pubs, sex, type, colors, ref) {
  ...
  if (sex == "M") {
    if (type[1]) {
      ...
      points(
        x = curve$x[curve$patient],
        y = curve$y[curve$patient],
        pch = 16,
        col = col
      )
    }
...
findSegments <- function(z) {
  ...
  z.old <- z[1]
  for (i in 1:length(z)) {
    if (!is.na(z[i]) && !is.na(z.old) && (z.old != z[i])) {
      k <- k + 1
    } # new segment
    if (!is.na(z[i])) {
      selector[k, i] <- TRUE
